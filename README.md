...
JetBrains Central takes precedence when the active model points at Central's local Wire proxy. The footer shows two bars: the Central budget with dollars used and the refill countdown, and today's locally tracked spend against a configurable daily limit. This applies to Anthropic, OpenAI, OpenAI Codex, and any future provider routed through the same proxy marker.
...
Authenticate direct providers through Pi's `/login` command. The extension resolves those credentials through Pi's provider API and does not read or write `auth.json`. For a Central-routed model, it runs `central limit --json` and does not use the direct provider credential.

## Install

Install the npm package:

Remove the upstream package first if it is already installed. Both packages own `/usage` and cannot load together:
...
When the active model belongs to a configured supported provider, the footer shows its available quota windows with reset countdowns. A Central-routed model always uses the Central budget instead of the underlying provider's direct subscription quota.
...
Run `/central-quota` to refresh a Central-routed model immediately.
...
The command writes the same setting that can be edited by hand in `~/.pi/agent/usage-bars.json`:
...
The Codex and Claude usage endpoints are fixed to their first-party services. Claude responses are cached briefly in the system temporary directory to coordinate multiple Pi processes and reduce rate limiting. The cache contains usage values, not credentials. Central usage comes from the local `central` executable.
...
Maintainers should follow the [release guide](docs/releasing.md). The first npm version is published manually; later tagged releases use npm trusted publishing through GitHub Actions.

## Credits

This extension is based on and inspired by:

- [hknet/pi-usage-bars](https://github.com/hknet/pi-usage-bars), the upstream project
...
This fork adds route-aware JetBrains Central usage and publishes under the `@jetserge` npm scope.
